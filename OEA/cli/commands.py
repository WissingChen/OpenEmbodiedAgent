...
from OEA import __logo__, __version__
from OEA.config.paths import get_workspace_path
from OEA.config.schema import Config
from OEA.utils.helpers import sync_workspace_templates

app = typer.Typer(
    name="OEA",
    help=f"{__logo__} OEA - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from OEA.config.paths import get_cli_history_path
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} OEA[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} OEA v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize OEA configuration and workspace."""
    from OEA.config.loader import get_config_path, load_config, save_config
    from OEA.config.schema import Config
    ...
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.OEA/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]OEA agent -m \"Hello!\"[/cyan]")
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from OEA.config.loader import load_config, set_config_path
...
def _print_deprecated_memory_window_notice(config: Config) -> None:
    ...
    if config.agents.defaults.should_warn_deprecated_memory_window:
        console.print(
            ...
            "[cyan]OEA onboard[/cyan] to refresh your config template."
        )
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show OEA runtime logs during chat"),
):
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]OEA is thinking...[/dim]", spinner="dots")
...
def channels_status():
    """Show channel status."""
    from OEA.channels.registry import discover_channel_names, load_channel_class
...
def _get_bridge_dir() -> Path:
    ...
    from OEA.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # OEA/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall OEA")
        raise typer.Exit(1)
...
def channels_login():
    ...
    from OEA.config.loader import load_config
    from OEA.config.paths import get_runtime_subdir
...
def status():
    """Show OEA status."""
    from OEA.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} OEA Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from OEA.providers.registry import PROVIDERS
