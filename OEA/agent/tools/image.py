...
class ImageTool(Tool):
    ...
    async def _execute_generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return (
                "Error: DASHSCOPE_API_KEY not found. Please set it in environment variables at ~/.OEA/workspace/.env. "
            ...
            for idx, img_url in enumerate(image_urls):
                async with httpx.AsyncClient(timeout=30.0) as download_client:
                    ...
                    if len(image_urls) > 1:
                        ...
                        path_obj = Path("~/.OEA/media") / image_path
                        save_path = path_obj.parent / f"{path_obj.stem}_{idx + 1}{path_obj.suffix}"
                    else:
                        save_path = Path("~/.OEA/media") / image_path
