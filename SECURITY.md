...
If you discover a security vulnerability in OEA, please report it by:
...
chmod 600 ~/.OEA/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.OEA/config.json` with file permissions set to `0600`
...
- ✅ Run OEA with a dedicated user account
...
- Keep authentication data in `~/.OEA/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for OEA and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash OEA
   sudo -u OEA OEA gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.OEA
   chmod 600 ~/.OEA/config.json
   chmod 700 ~/.OEA/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.OEA/logs/OEA.log
   ```
...
- **Chat history is stored locally** - protect the `~/.OEA` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.OEA/logs/OEA.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/OEA/security/advisories
- Release Notes: https://github.com/HKUDS/OEA/releases
