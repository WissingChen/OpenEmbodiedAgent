...
import OEA.channels.matrix as matrix_module
from OEA.bus.events import OutboundMessage
from OEA.bus.queue import MessageBus
from OEA.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from OEA.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("OEA.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "OEA.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("OEA.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "OEA.channels.matrix.asyncio.create_task", _fake_create_task
    )
